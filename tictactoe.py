...
# randomly selection of first player
player = random.choice(["X", "O"])

# define the board
board = [["_" for i in range(3)] for j in range(3)]

# define the player's move so cannot override another player's position
def make_move(board, player, row, col):
    if board[row][col] == "_":
        board[row][col] = player
    else:
        print("Please choose another position on the board next time.")
